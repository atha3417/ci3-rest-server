...
|
*/
$conn = mysqli_connect('localhost', 'root', '', 'php_rest-server');
$query = mysqli_query($conn, "SELECT email, password FROM user WHERE is_active = 1");
$config['rest_valid_logins'] = [];
foreach ($query as $q) {
	$config['rest_valid_logins'] += [$q['email'] => $q['password']];
}
/*
|--------------------------------------------------------------------------
